...
def draw_lanes(img, lines, color=[255, 0, 0], thickness=8):
    ...
    left_points = [(x1, y1) for line in left_lines for x1, y1, x2, y2 in line]  #start

    left_points = left_points + [(x2, y2)
                                 for line in left_lines for x1, y1, x2, y2 in line]  #end
...
def least_squares_fit(point_list, ymin, ymax):
    ...
    fit = np.polyfit(y, x, 1)  #reverse the x and y

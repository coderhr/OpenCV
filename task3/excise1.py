...
if __name__ == "__main__":
    output = 'test_videos/cv2_yellow_lane_green_mark.avi'
    capture = cv2.VideoCapture("test_videos/cv2_yellow_lane.mp4")
    fourcc = cv2.VideoWriter_fourcc(*'DIVX')
    outfile = cv2.VideoWriter(output, fourcc, 25, (960, 540))
    while (capture.isOpened()):
        ret, frame = capture.read()
        out_clip = process_an_image(frame)
        outfile.write(out_clip)